...
// OAuth credentials as SeedBot.  The endpoint uses conversation_engine
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as SeedBot — available when the
...
export async function connectVoiceStream(
  ...
  // visibility.external: true). We target that host instead of forkit.chat
  // because the forkit.chat CF zone uses TLS fingerprinting and challenges
  ...
  // Desktop dictation still uses forkit.chat (Swift URLSession has a
