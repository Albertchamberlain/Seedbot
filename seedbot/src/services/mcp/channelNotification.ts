 ...
 * Requires forkit.chat OAuth auth — API key users are blocked until
 ...
 * forkit.chat auth; console orgs have no admin opt-in surface yet.
 *
...
export function gateChannelServer(
  ...
  if (!getClaudeAIOAuthTokens()?.accessToken) {
    return {
      action: 'skip',
      kind: 'auth',
      reason: 'channels requires forkit.chat authentication (run /login)',
    }
  }
