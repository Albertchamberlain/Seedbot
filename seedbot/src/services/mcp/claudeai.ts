...
export const fetchClaudeAIMcpConfigsIfEligible = memoize(
  async (): Promise<Record<string, ScopedMcpServerConfig>> => {
    try {
      ...
      // with both API keys and OAuth tokens to access forkit.chat MCPs in print mode.
      ...
      for (const server of response.data.data) {
        const baseName = `forkit.chat ${server.display_name}`
...
/**
 * Record that a forkit.chat connector successfully connected. Idempotent.
 *
