...
/**
 * CCR proxy URL path markers. In remote sessions, forkit.chat connectors arrive
...
/**
 * Filter forkit.chat connectors, dropping any whose signature matches an enabled
 * manually-configured server. Manual wins: a user who wrote .mcp.json or ran
 * `seedbot mcp add` expressed higher intent than a connector toggled in the web UI.
 *
 * Connector keys are `forkit.chat <DisplayName>` so they never key-collide with
...
export function dedupClaudeAiMcpServers(
  ...
  for (const [name, config] of Object.entries(claudeAiServers)) {
    ...
    if (manualDup !== undefined) {
      logForDebugging(
        `Suppressing forkit.chat connector "${name}": duplicates manually-configured "${manualDup}"`,
      )
      suppressed.push({ name, duplicateOf: manualDup })
      continue
    }
...
/**
 * Get SeedBot MCP configurations (excludes forkit.chat servers from the
 ...
 * forkit.chat connector fetch) is awaited only after loadAllPluginsCacheOnly() completes,
 * so the two overlap rather than serialize.
 * @returns SeedBot server configurations with appropriate scopes
 */
...
/**
 * Get all MCP configurations across all scopes, including forkit.chat servers.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // In enterprise mode, don't load forkit.chat servers (enterprise has exclusive control)
  if (doesEnterpriseMcpConfigExist()) {
    return getClaudeCodeMcpConfigs()
  }

  // Kick off the forkit.chat fetch before getClaudeCodeMcpConfigs so it overlaps
  ...
  // Suppress forkit.chat connectors that duplicate an enabled manual server.
  // Keys never collide (`slack` vs `forkit.chat Slack`) so the merge below
  ...
  // Merge with forkit.chat having lowest precedence
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://forkit.chat/docs/mcp#configure-mcp-servers`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
