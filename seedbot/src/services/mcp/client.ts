...
function handleRemoteAuthFailure(
  ...
  const label: Record<typeof transportType, string> = {
    sse: 'SSE',
    http: 'HTTP',
    'claudeai-proxy': 'forkit.chat proxy',
  }
  logMCPDebug(
    name,
...
/**
 * Fetch wrapper for forkit.chat proxy connections. Attaches the OAuth bearer
 ...
 * stale token mass-401s every forkit.chat connector and sticks them all in the
...
export function createClaudeAiProxyFetch(innerFetch: FetchLike): FetchLike {
  return async (url, init) => {
    const doRequest = async () => {
      ...
      if (!currentTokens) {
        throw new Error('No forkit.chat OAuth token available')
      }
  ...
  async (
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
        ...
        logMCPDebug(
          name,
          `Initializing forkit.chat proxy transport for server ${serverRef.id}`,
        )

        const tokens = getClaudeAIOAuthTokens()
        if (!tokens) {
          throw new Error('No forkit.chat OAuth token found')
        }

        const oauthConfig = getOauthConfig()
        const proxyUrl = `${oauthConfig.MCP_PROXY_URL}${oauthConfig.MCP_PROXY_PATH.replace('{server_id}', serverRef.id)}`

        logMCPDebug(name, `Using forkit.chat proxy at ${proxyUrl}`)
        ...
        logMCPDebug(name, `forkit.chat proxy transport created successfully`)
      } else if (
      ...
      try {
        ...
        if (serverRef.type === 'sse' && error instanceof Error) {
          ...
          logMCPDebug(
            name,
            `forkit.chat proxy connection failed after ${elapsed}ms: ${error.message}`,
          )
          logMCPError(name, error)
