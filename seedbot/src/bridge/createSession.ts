 ...
 *
 * Used by both `seedbot remote-control` (empty session so the user has somewhere to
 ...
 * explicit client action. Both `seedbot remote-control` (standalone bridge) and the
 ...
 * connection is active, so the title stays in sync on forkit.chat.
 *
