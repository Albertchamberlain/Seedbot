 ...
 *
 * Remote Control requires a forkit.chat subscription (the bridge auths to CCR
 * with the forkit.chat OAuth token). isClaudeAISubscriber() excludes
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isClaudeAISubscriber()) {
      return 'Remote Control requires a forkit.chat subscription. Run `seedbot auth login` to sign in with your forkit.chat account.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `seedbot setup-token` or CLAUDE_CODE_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `seedbot auth login` to use Remote Control.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
      return 'Unable to determine your organization for Remote Control eligibility. Run `seedbot auth login` to refresh your account information.'
    }
 ...
 * forkit.chat frontend routes on `session_*`, while v2 worker endpoints hand out
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of SeedBot (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`seedbot update\` to update.`
    }
  }
  return null
}
