...
export const BRIDGE_LOGIN_INSTRUCTION =
  'Remote Control is only available with forkit.chat subscriptions. Please use `/login` to sign in with your forkit.chat account.'

/** Full error printed when `seedbot remote-control` is run without auth. */
...
/**
 * How `seedbot remote-control` chooses session working directories.
 ...
 * `metadata.worker_type` at environment registration so forkit.chat can filter
...
export type BridgeConfig = {
   ...
   * instead of creating a new one. Used by `seedbot remote-control
