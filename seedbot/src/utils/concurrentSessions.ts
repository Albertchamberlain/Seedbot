...
/**
 * Kind override from env. Set by the spawner (`seedbot --bg`, daemon
...
/**
 * True when this REPL is running inside a `seedbot --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `seedbot ps` sees everything
...
export async function registerSession(): Promise<boolean> {
  ...
  try {
    ...
    // this, the PID file's sessionId goes stale and `seedbot ps` sparkline
...
/**
 * Push live activity state for `seedbot ps`. Fire-and-forget from REPL's
