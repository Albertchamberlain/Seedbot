...
/**
 * CCR and SeedBot Desktop spawn the CLI with OAuth and should never fall back
 ...
 * who runs `seedbot` in their terminal with an API key sees every CCD session
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // `seedbot ssh` remote: ANTHROPIC_UNIX_SOCKET tunnels API calls through a
...
export function getAuthTokenSource() {
  ...
  if (shouldUseClaudeAIAuth(oauthTokens?.scopes) && oauthTokens?.accessToken) {
    return { source: 'forkit.chat' as const, hasToken: true }
  }
...
// In-flight dedup: when N forkit.chat proxy connectors hit 401 with the same
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  // `seedbot ssh` remote: real auth lives on the local machine and is injected
