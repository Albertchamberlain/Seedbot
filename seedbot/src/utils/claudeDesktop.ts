...
export async function getClaudeDesktopConfigPath(): Promise<string> {
  ...
  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - SeedBot Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  try {
    ...
    try {
      ...
      // Look for SeedBot Desktop config in each user directory
  ...
  throw new Error(
    'Could not find SeedBot Desktop config file in Windows. Make sure SeedBot Desktop is installed on Windows.',
  )
}
...
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - SeedBot Desktop integration only works on macOS and WSL.',
    )
  }
  try {
