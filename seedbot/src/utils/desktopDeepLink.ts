...
/**
 * Builds a deep link URL for SeedBot Desktop to resume a CLI session.
...
/**
 * Check if SeedBot Desktop app is installed.
...
/**
 * Detect the installed SeedBot Desktop version.
...
/**
 * Build and open a deep link to resume the current session in SeedBot Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'SeedBot Desktop is not installed. Install it from https://forkit.chat',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open SeedBot Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
