...
const APP_NAME = 'SeedBot URL Handler'
const DESKTOP_FILE_NAME = 'claude-code-url-handler.desktop'
const MACOS_APP_NAME = 'SeedBot URL Handler.app'
 ...
 * already-installed (and signed) `seedbot` binary. When macOS opens a
 * `claude-cli://` URL, it launches `seedbot` through this app bundle.
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>SeedBot Deep Link</string>
...
Name=${APP_NAME}
Comment=Handle ${DEEP_LINK_PROTOCOL}:// deep links for SeedBot
 ...
 * points at the expected `seedbot` binary. Reads the registration artifact
