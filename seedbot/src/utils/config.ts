...
export type ProjectConfig = {
  ...
  /** Spawn mode for `seedbot remote-control` multi-session. Set by first-run dialog or `w` toggle. */
...
export type GlobalConfig = {
  ...
  mcpServers?: Record<string, McpServerConfig>
  // forkit.chat MCP connectors that have successfully connected at least once.
  ...
  firstStartTime?: string // ISO timestamp when SeedBot was first started on this machine
  ...
  // SeedBot usage tracking
  claudeCodeFirstTokenDate?: string // ISO timestamp of the user's first SeedBot OAuth token
  ...
  // SeedBot hint protocol state (<claude-code-hint /> tags from CLIs/SDKs).
  ...
  // cold API call on every `seedbot -p` invocation.
  metricsStatusCache?: {
    enabled: boolean
    timestamp: number
  }
