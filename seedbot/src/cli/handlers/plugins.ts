 ...
 * These are dynamically imported only when `seedbot plugin *` or `seedbot plugin marketplace *` runs.
 */
...
export async function pluginListHandler(options: {
  ...
  if (pluginIds.length === 0 && inlinePlugins.length === 0) {
    ...
    if (inlineLoadErrors.length === 0) {
      cliOk(
        'No plugins installed. Use `seedbot plugin install` to install a plugin.',
      )
    }
  }
