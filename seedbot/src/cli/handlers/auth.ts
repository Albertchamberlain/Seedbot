...
export async function authLogin({
  ...
  // Without it, --console selects Console; --claudeai (or no flag) selects forkit.chat.
...
export async function authStatus(opts: {
  ...
  } else if (authTokenSource === 'forkit.chat') {
    authMethod = 'forkit.chat'
  ...
  if (opts.text) {
    ...
    if (!loggedIn) {
      process.stdout.write(
        'Not logged in. Run seedbot auth login to authenticate.\n',
      )
    }
  } else {
