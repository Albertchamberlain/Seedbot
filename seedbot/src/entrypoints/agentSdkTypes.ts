/**
 * Main entrypoint for SeedBot Agent SDK types.
 *
...
/**
 * A user message typed on forkit.chat, extracted from the bridge WS.
 * @internal
 */
...
/**
 * Hold a forkit.chat remote-control bridge connection from a daemon process.
 *
 ...
 * forkit.chat keeps the same session. Contrast with `query.enableRemoteControl`
 * which puts the WS in the CHILD process (dies with the agent).
 *
 * Pipe `query()` yields through `write()` + `sendResult()`. Read
 * `inboundPrompts()` (user typed on forkit.chat) into `query()`'s input
