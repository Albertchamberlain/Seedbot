...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (SeedBot)`);
    return;
  }
  ...
  // Fast-path for `seedbot remote-control` (also accepts legacy `seedbot remote` / `seedbot sync` / `seedbot bridge`):
  ...
  // Fast-path for `seedbot daemon [subcommand]`: long-running supervisor.
  ...
  // Fast-path for `seedbot ps|logs|attach|kill` and `--bg`/`--background`.
  ...
  // Fast-path for `seedbot environment-runner`: headless BYOC runner.
  // feature() must stay inline for build-time dead code elimination.
  ...
  // Fast-path for `seedbot self-hosted-runner`: headless self-hosted-runner
