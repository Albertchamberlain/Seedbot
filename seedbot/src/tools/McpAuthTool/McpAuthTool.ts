...
export function createMcpAuthTool(
  ...
  return {
    ...
    async call(_input, context) {
      // forkit.chat connectors use a separate auth flow (handleClaudeAIAuth in
      ...
      if (config.type === 'claudeai-proxy') {
        return {
          data: {
            ...
            message: `This is a forkit.chat MCP connector. Ask the user to run /mcp and select "${serverName}" to authenticate.`,
          },
        }
      }
